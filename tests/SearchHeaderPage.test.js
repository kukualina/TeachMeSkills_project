...
import { MainPage, ResultSearchPage, SearchHeaderPage, BasePage } from '../src/pages/index';
...
test.describe('Осуществить поиск элементов', () => {
  test('Осуществить поиск с главной страницы', async ({ page }, testInfo) => {
    ...
    await allure.step("Поисковый запрос 'Пушкин' включен в строку url", async () => {
      const currentUrl = await page.url();
      expect(currentUrl).toContain('q=%D0%9F%D1%83%D1%88%D0%BA%D0%B8%D0%BD');
    });
  });

  test('Осуществить повторный поиск с главной страницы', async ({ page }, testInfo) => {
    const basePage = new BasePage(page);
    await basePage.open('https://shop.tretyakovgallery.ru/');
    await basePage.closePopup();
    const searchHeaderPage = new SearchHeaderPage(page);
    ...
    await allure.step("Поисковый запрос 'Сказки' включен в строку url", async () => {
      const currentUrl = await page.url();
      expect(currentUrl).toContain('q=%D0%A1%D0%BA%D0%B0%D0%B7%D0%BA%D0%B8');
    });
  });
});
